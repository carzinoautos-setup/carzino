  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg lg:rounded-xl overflow-hidden hover:shadow-lg transition-shadow vehicle-card flex flex-col h-full">
      <div className="relative">
        ...
        {vehicle.images && vehicle.images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-70 hover:bg-opacity-85 text-white p-1.5 rounded-full transition-all z-10"
            >
              <ChevronLeft className="w-4 h-4 text-white" />
            </button>
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-70 hover:bg-opacity-85 text-white p-1.5 rounded-full transition-all z-10"
            >
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="flex gap-2 mb-1.5 items-center justify-between">
        ...
        <h3 className="carzino-vehicle-title text-gray-900 mb-1.5 leading-tight overflow-hidden whitespace-nowrap text-ellipsis">
          {vehicle.title}
        </h3>
