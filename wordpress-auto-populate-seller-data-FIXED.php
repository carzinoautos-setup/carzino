<?php
/**
 * Carzino - Auto-Populate Missing Seller Data (FIXED VERSION)
...
add_filter('woocommerce_rest_prepare_product_object', 'carzino_ensure_seller_data_fixed', 20, 3);

function carzino_ensure_seller_data_fixed($response, $product, $request) {
    ...
    if (empty($existing_seller_name)) {
        ...
        if (!empty($account_number)) {
            ...
            $seller_data = carzino_get_seller_data_by_account_fixed($account_number);
...
/**
 * Get seller data by account number (FIXED VERSION)
 */
function carzino_get_seller_data_by_account_fixed($account_number) {
    ...
    // FIXED: More robust query with proper post type checking
    $seller_post = $wpdb->get_row($wpdb->prepare("
        ...
        AND (
            (pm.meta_key = 'account_number_seller' AND pm.meta_value = %s)
            OR p.post_title LIKE %s
        )
        ORDER BY pm.meta_key DESC
        LIMIT 1
    ", $account_number, '%' . $wpdb->esc_like($account_number) . '%'));
    ...
    if ($seller_post) {
        ...
        // Map common field names with better fallbacks
        $field_mapping = array(
            'business_name' => 'acount_name_seller',
            'company_name' => 'acount_name_seller', 
            'name' => 'acount_name_seller',
            'acount_name_seller' => 'acount_name_seller',
            'account_name_seller' => 'acount_name_seller',
            'city' => 'city_seller',
            'city_seller' => 'city_seller',
            'state' => 'state_seller',
            'state_seller' => 'state_seller',
            'zip' => 'zip_seller',
            'zip_seller' => 'zip_seller',
...
// FIXED: Better error handling for bulk population endpoint
add_action('rest_api_init', function() {
    register_rest_route('carzino/v1', '/populate-seller-data', array(
        'methods' => 'GET',
        'callback' => 'carzino_populate_all_seller_data_fixed',
        'permission_callback' => '__return_true'
    ));
});

function carzino_populate_all_seller_data_fixed($request) {
    global $wpdb;
    
    // FIXED: Better error handling and limits
    try {
        ...
        foreach ($products_missing_seller_data as $product) {
            ...
            try {
                $seller_data = carzino_get_seller_data_by_account_fixed($product->account_number);
...
error_log('CARZINO AUTO-POPULATE FIXED: Seller data auto-population loaded successfully');
?>
