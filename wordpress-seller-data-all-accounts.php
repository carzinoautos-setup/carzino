<?php
/**
 * FIXED Carzino - Product API Seller Enhancement (ALL ACCOUNTS)
 * This version handles ALL seller accounts, not just account "73"
 */

// Hook into WooCommerce REST API
add_filter('woocommerce_rest_prepare_product_object', 'carzino_add_seller_data_all_accounts', 10, 3);

function carzino_add_seller_data_all_accounts($response, $product, $request) {
    try {
        ...
        if (!empty($account_number) && isset($seller_database[$account_number])) {
            ...
            error_log("CARZINO ALL ACCOUNTS: Added seller data for account $account_number ({$seller_data['account_name']}) to product $product_id");
        } else {
            // Log when account number is found but no seller data
            if (!empty($account_number)) {
                error_log("CARZINO ALL ACCOUNTS: Account $account_number found but no seller data defined for product $product_id");
            }
        }
        
    } catch (Exception $e) {
        error_log('CARZINO ALL ACCOUNTS ERROR: ' . $e->getMessage());
    }
...
error_log('CARZINO ALL ACCOUNTS: Seller data enhancement loaded for all accounts (73, 101, 205, 312, 445)');
?>
